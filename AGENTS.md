...
## Image quality check (capture gate) — spec & status: IMPLEMENTED
- **Where it runs:** `ScanReviewScreen` → "Analyze". After crop (`cropAndSave`), on the cropped JPEG, before navigation to inference. Covers both camera and gallery paths (both flow through the review screen). Runs off the main thread (`Dispatchers.Default`); the Analyze button disables ("Checking…") while it runs (in-flight guard against double-taps; any exception fails open to Analyze).
- **What it measures** (`feature/scan/ImageQuality.kt`): the checker decodes the photo **downsampled** via `BitmapFactory.Options.inSampleSize` (never allocates the full-resolution bitmap — same ANR rule as inference) targeting a ≤ 256px analysis edge, recycles every decoded/downscaled copy, extracts Rec.601 grayscale, then:
  - **Blur** — variance of the 3×3 Laplacian response over interior pixels; flat/no-edge images → ~0.
  - **Exposure** — mean grayscale luminance (0..255).
  - **Size** — smallest edge of the ORIGINAL photo (from `inJustDecodeBounds`; model input is 224×224).
- **Thresholds** (`QualityThresholds`, all tunable): `BLURRY` if Laplacian variance < **80**; `TOO_DARK` if mean luminance < **50**; `TOO_BRIGHT` if mean luminance > **215**; `TOO_SMALL` if min dimension < **480px**.
- **Behavior — soft gate, never hard-block** (screening-aid philosophy): issues show an amber banner (`DermoColors.Bloom`/`AmberText`) with per-issue message + first-issue guidance and two actions: **Analyze anyway** (proceeds) / **Adjust crop** (dismisses). A clean report navigates straight to results. Decode failure fails OPEN (empty report) — the existing inference error state handles unreadable files.
- **Testability:** pixel math (`ImageQualityMetrics`) and threshold decision (`ImageQualityDecision`) are pure Kotlin — unit-tested on the JVM (`feature/scan/src/test/.../ImageQualityTest.kt`, 13 tests, no Robolectric). `ImageQualityChecker.assessFile` is the thin `android.graphics.Bitmap` adapter.
- Verified: `:feature:scan:testDebugUnitTest` green (13/13), `assembleDebug` green.
